...
var $formTarget=$(sFormSelector);var $domTarget=$(sFormSelector).parent();if($formTarget.data('sendform-reponse-selector')){$domTarget=$($formTarget.data('sendform-reponse-selector'));}
oParams=$.extend($formTarget.data(),obj.data());oParams.parameters=$formTarget.serializeJSON();if($(sFormSelector+' div[contenteditable=true]').size()!=0){$(sFormSelector+' .ui-editor').each(function(){sInputName=$(this).data('name');sInputValue=$(this).parent().find('div[contenteditable=true]:first').html();oParams.parameters.sInputName=sInputValue;});}
...
if(obj.hasClass('closeModalOnCallback')){Ux.closeModal();}}});},loadView:function(){if($('.ui-loadable').size()>0){$('.ui-loadable').each(function(){var sUrlTarget='';if(typeof($(this).data('bundle'))!=='undefined'&&typeof($(this).data('controller'))!=='undefined'&&typeof($(this).data('action'))!=='undefined'){sUrlTarget='/'+$(this).data('bundle')+'/'+$(this).data('controller')+'/'+$(this).data('action');}else if(typeof($(this).data('url'))!=='undefined'){sUrlTarget=$(this).data('url');}else{Ux.sendNotification('No url specified to load ui-loadable div #'+$(this).attr('id'),'error','glyphicon glyphicon-warning',false);return false;}
var sSelector='#'+$(this).attr('id');var aData=$(sSelector).data();$.ajax({type:'POST',url:sUrlTarget,data:aData,beforeSend:function(preload){$(this).data('initialContent',$(this).html());$(sSelector).empty();},success:function(rep){$(sSelector).append(rep.content);$('#activityDebug').append(rep.debug);},error:function(err){$(sSelector).append($(sSelector).data('initialContent'));},complete:function(){$(sSelector).removeData('initialContent');}});});}else{this.fireUx();}},reload:function(oItem){var sUrlTarget='';if(typeof(oItem.data('bundle'))!=='undefined'&&typeof(oItem.data('controller'))!=='undefined'&&typeof(oItem.data('action'))!=='undefined'){sUrlTarget='/'+oItem.data('bundle')+'/'+oItem.data('controller')+'/'+oItem.data('action');}else if(typeof(oItem.data('url'))!=='undefined'){sUrlTarget=oItem.data('url');}else{Ux.sendNotification('No url specified to load ui-loadable div #'+oItem.attr('id'),'error','glyphicon glyphicon-warning',false);return false;}
var sSelector='#'+oItem.attr('id');var aData=$(sSelector).data();$.ajax({type:'POST',url:sUrlTarget,data:aData,beforeSend:function(preload){oItem.data('initialContent',oItem.html());$(sSelector).empty();$container.data('grid-loaded',false);Ux.sendNotification('Chargement en cours...','info','glyphicon glyphicon-info-sign');},success:function(rep){if(rep.status===1){$(sSelector).append(rep.content);$('#activityDebug').append(rep.debug);}},error:function(err){$(sSelector).append($(sSelector).data('initialContent'));},complete:function(){$(sSelector).removeData('initialContent');Ux.hideNotifications();}});},initGrids:function(){var iColumnsIndex=0;var iColumnsCount=6;var iTwitterBootstrapGridClass=2;$('.ui-grid').each(function(){var sGridSelector='#'+$(this).attr('id');if(!$(this).data('grid-loaded')){if(parseInt($(this).data('columns'))>0){iColumnsCount=parseInt($(this).data('columns'));iTwitterBootstrapGridClass=12%iColumnsCount;}
...
$(sGridSelector+' .ui-grid-item.notSorted').each(function(){if((iColumnsIndex+1)>iColumnsCount){iColumnsIndex=0;}
$(sGridSelector+' .ui-grid-column').eq(iColumnsIndex).append($(this));iColumnsIndex++;$(this).data('curCol',iColumnsIndex);$(this).removeClass('notSorted');});});},initForms:function(){$('form').each(function(){if(!$(this).data('HasChangesListnenerFired')){$(this).data('bHasChange',false);$('body').on('change',$(this).attr('id'),function(){$(this).data('bHasChange',true);});$(this).data('HasChangesListnenerFired',true);}});this.initEditableElements();this.initEditors();this.initCheckbox();this.initDatepicker();},initCheckbox:function(){$('.ui-checkbox').each(function(){if(!$(this).data('ui-checkbox-fired')){$(this).data('ui-checkbox-fired',true);$(this).bootstrapSwitch();}});},initDatepicker:function(){$('.ui-datetimepicker').each(function(){if(!$(this).data('uiDateTimePickerFired')){$('.ui-datetimepicker').datetimepicker();$(this).data('uiDateTimePickerFired',true);}});},initCarousels:function(){$('.ui-carousel').each(function(){if(!$(this).data('carouselFired')){var iAutoplay=(($(this).hasClass('noAutoplay'))?0:5000);$(this).carousel({interval:iAutoplay,duration:500});$(this).data('carouselFired',true);}});},registerBodyListeners:function(){if(!$('body').data('UxListened')){$('body').on('focus','[placeholder]',function(){Ux.sendNotification($(this).attr('placeholder'),'info','glyphicon glyphicon-info-sign');});$('body').on('blur','[placeholder]',function(){Ux.hideNotifications();});$('body').on('click','.ui-sendxhr',function(){Ux.sendXHR($(this));});$('body').on('change','.ui-sendxhronchange',function(){Ux.sendXHR($(this));});$('body').on('click','.ui-sendform',function(){Ux.sendForm($(this));});$('.ui-sendform').each(function(){var $btn=$(this);$($(this).data('form')).on('submit',function(){$btn.trigger('click');return false;});});$('.ui-reload').on('click',function(){if(typeof($(this).data('sreloadtarget'))!=='undefined'){Ux.loadView($($(this).data('sreloadtarget')));}else{Ux.loadView();}
return false;});$('body').on('mouseenter mouseleave','.ui-flip',function(){$(this).toggleClass('flip');});$('.ui-clickflip').toggle(function(){$(this).addClass('flip');},function(){$(this).removeClass('flip');});$('body').on('click','.ui-confirm',function(){if(!confirm('Etes vous sure?')){return false;}});$('body').on('click','.ui-select-all',function(){var sCheckboxSelector=$(this).data('checkbox-class');var bCheckState=$(this).is(':checked');if(typeof(sCheckboxSelector)!=='undefined'){$('.ui-select.'+sCheckboxSelector).prop('checked',bCheckState);if($($(this).data('select-parent')+' .ui-select:checked').size()>1){$($(this).data('delete-selector')).removeClass('hide');}else{$($(this).data('delete-selector')).addClass('hide');}}});$('body').on('click','.ui-select',function(){if($(this).data('delete-selector')!=='undefined'&&$(this).data('select-parent')!=='undefined'){if($($(this).data('select-parent')+' .ui-select:checked').size()>1){$($(this).data('delete-selector')).removeClass('hide');}else{$($(this).data('delete-selector')).addClass('hide');}}});$('body').on('click','.ui-toggle',function(){if($($(this).data('toggle-selector')).size()!==0){$($(this).data('toggle-selector')).toggleClass('hide');}});$('.modal').on('hide',function(){var sSelector=$(this).attr('id');if($(sSelector+' form').size()!==0){$.each($(sSelector+' form'),function(){if($(this).data('bHasChange')){alert('Warning!');}});}});$('body').on('.ui-toggle','click',function(){Ux.toggle($($(this).data('toggle-selector')));});$('.ui-toggle-popover').popover({container:'body',placement:'auto',html:true,animation:true,content:function(){return $($(this).data('popover')).html();}});$('.ui-tip').tooltip({placement:'auto',delay:{show:0,hide:0}});$('body').on('click','.ui-focus',function(){$($(this).data('focus-selector')).focus();});$('.ui-loadscroll').bind('scroll',function(){if($(this).scrollTop()===($(this).prop('scrollHeight')-$(this).outerHeight())){if($(this).find('.ui-scroll-loadable')){$('.ui-scroll-loadable').each(function(){Ux.loadScroll($(this));});}}
return false;});$('body').data('UxListened',true);}},registerListeners:function(){$('.ui-equalize-height').each(function(){$(this).data('equalized-height',$(this).parent('.ui-equalize-height .ui-equalize:first').height());$('.ui-equalize').each(function(){if($(this).height()>maxHeight){$(this).parent('.ui-equalize-height').data('equalizd-height',$(this).height());}
$(this).height($(this).parent('.ui-equalize-height').data('equalizd-height'));});});},initContentFlow:function(){$('.ui-contentflow').each(function(){if($(this).attr('id')==='undefined'){Ux.sendNotification('No id attribute on a ui-contentflow widget!','error','glyphicon glyphicon-error-sign',false);}else{var sNodeId=$(this).attr('id');var sConnectedCarousel=null;if(typeof $(this).data('connected-carousel')!=='undefined'){sConnectedCarousel=$(this).data('connected-carousel');}
var oContentFlow=new ContentFlow(sNodeId,{visibleItems:10,endOpacity:0.6,startItem:$('#'+sNodeId+' div[data-carousel=2]').index(),maxItemHeight:240,scaleFactorLandscape:1,onReachTarget:function(item){$('.details').addClass('hide');$($('#'+sNodeId+' div.item').eq(item.index).data('toggle-selector')).toggleClass('hide');},onclickActiveItem:function(item){if(sConnectedCarousel!==null){$(sConnectedCarousel).carousel($('#'+sNodeId+' div.item').eq(item.index).data('carousel'));}},onclickInactiveItem:function(item){if(sConnectedCarousel!==null){$(sConnectedCarousel).carousel($('#'+sNodeId+' div.item').eq(item.index).data('carousel'));}}});if(typeof $(this).data('connected-carousel')!=='undefined'){$(sConnectedCarousel).data('contentflow',oContentFlow);}}});},onLoad:function(){this.registerBodyListeners();this.registerListeners();this.loadView();},onAjaxStop:function(){this.fireUx();},onUnload:function(){},fireUx:function(){this.initAppLayout();this.initGrids();this.initForms();this.formatTimestamps();this.initCarousels();this.initContentFlow();}};return Ux;};})(jQuery);$(document).ready(function(){var Ux=$.fn.userExperience();Ux.onLoad();$(document).ajaxStop(function(){Ux.onAjaxStop();});$(window).unload(function(){Ux.onUnload();});});