...
class core_Email
{
    ...
    /**
     * Constructeur de la classe Kernel_Email
     * @param type $iEmailId
     * @param type $sEmail
     */
    public function __construct($iEmailId, $sEmail, $iCountryId, $iLanguageId)
    {
        assert('is_null($iEmailId) || validator::integer($iEmailId, 1) === validator::STATUS_OK');

        $this->oDbTmpEmail = new db_Email_langues_pays();

        try {
            $this->oDbTmpEmail->loadByCountryLanguage($iEmailId, $iCountryId, $iLanguageId);
        } catch (CoreObjectException $oException) {
    ...
    {
        assert('isset($aParameters["email"]) && validator::email($aParameters["email"]) === validator::STATUS_OK');
    ...
    protected function addTrackingMails($iEmailId, $sEmail)
    {
        ...
        $oMember = new db_Membres();
        $oTrackingMails = new db_Tracking_mails();
        ...
        $sContent = preg_replace_callback(
            ...
            function(array $aMatches)
            {
                ...
                return str_replace(
                    $aMatches[1],
                    'http://fr.bazarchic.com/redirection_tracking.php?track=' . core_Email::crypto($iTrackingMailId . '##' . $aMatches[1]),
                    $aMatches[0]
                );
            },
